...
struct DNASequences *ReadFastaFile(char *file)
{
	int size,i,j,k,FirstFlag;
	char *readIn;
	FILE *fp;
...
void usage()
{
	...
	printf("-abundance <abundance_file>     [char]this file contains the source genomes for simulation and their abundance.\n");
	printf("-index <index_file>             [char]the index file used in simulation to supply information about file paths and gene number.\n");
	printf("-o <output_file>                [char]prefix of output file.\n\n");
	...
	printf("-config <config_file>           [char]the configure file for sequencing, default is the 'simulation.config'.\n");
	printf("-seed <seed>                    [int]the seed used in random, default 0.\n");
	printf("-strandspecific                 simulate strand-specific RNA-seq sequencing, default is unstrand-specific.\n");
	printf("-express <express_per>          [float]the percentage of expression gene in one organism, default 0.1.\n");
	printf("-gene <gene_file>               [char]this file contains the list of expression genes and their abundance.\n\n");
	printf("-rRNA <percentage>		[float]the percentage of rRNA reads in all simulated reads, default 0.2.\n");

	printf("[Simulate differential expression]:\n");
	printf("-diff <differential expression> simulate differential expression.\n");
	printf("-up <up_expression_per>         [float]the percentage of up expression genes in all expressed genes, default 0.2.\n");
	printf("-down <down_expression_per>     [float]the percentage of down expression genes in all expressed genes, default 0.2.\n");
	...
	printf("-more <more_abundance_per>	[float]the percentage of genomes whose relative abundance in control are more than in treat group, default 0.2.\n");
	printf("-less <down_abundance_per>	[float]the percentage of genomes whose relative abundance in control are less than in treat group, default 0.2.\n");
	...
	printf("-replicate <replicates>         [int]the number of biological replicates. By default, replicates is 3 in differential expression simulation and replicates is 1 for other simulation.\n");
	printf("-prob <probability>             [float]the probability is used in the negative binomial to simulate number of reads in replicates, the smaller of probability, the more dispersion of reads count, default 0.8.\n");
	return;
}
...
//default parameters
	...
	up_expression_per=0.2;
	down_expression_per=0.2;
	maxFC=20;
	minFC=2;
	replicates=-1;
	pnbinom=0.8;
...
//start rRNA
	if(rRNA>0)  //only those have rRNA fasta file
	{
		...
		while(nodeGenome!=NULL)
		{
			...
			if(nodeGenome->rRNA_path!=NULL)
			{
				headDNASequences=ReadFastaFile(nodeGenome->rRNA_path);
...
//start mRNA
	...
	//cDNA & simulate
		...
		if(nodeGenome->transcript_path!=NULL)
		{
			headDNASequences=ReadFastaFile(nodeGenome->transcript_path);
			if(headDNASequences==NULL)
			{
		...
		{
			headDNASequences=ReadFastaFile(nodeGenome->genome_path);
			if(headDNASequences==NULL)
			{
